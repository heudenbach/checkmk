...
def discover_batteria_wasserm(section):
    yield Service()

def check_batteria_wasserm(section):
...
register.check_plugin(
    name = "batteria_wasserm",
    service_name = "Batterieraum A - Wassermelder",
    discovery_function = discover_batteria_wasserm,
    check_function = check_batteria_wasserm,
)
...
register.snmp_section(
    ...
    exists(".1.3.6.1.3.1.0.25")
        ...
        base = '.1.3.6.1.3.1.4',
        oids = [
            '6.0',    # Batterieraum A - Wassermelder
        ],
    ),
)