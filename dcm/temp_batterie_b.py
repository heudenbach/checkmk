...
def discover_temp_batterie_b(section):
    yield Service()

def check_temp_batterie_b(section):
    ...
    yield Result(
        state=state,
        summary="Temperatur Batterie B: % 6.1f° Celcius" % (aussen_temp)
    )
    return

register.check_plugin(
    name = "temp_batterie_b",
    service_name = "Batterie B Temperatur",
    discovery_function = discover_temp_batterie_b,
    check_function = check_temp_batterie_b,
)
