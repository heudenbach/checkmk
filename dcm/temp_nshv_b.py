...
def discover_temp_nshv_b(section):
    yield Service()

def check_temp_nshv_b(section):
    ...
    yield Result(
        state=state,
        summary="Temperatur NSHV B: % 6.1f° Celcius" % (aussen_temp)
    )
    return

register.check_plugin(
    name = "temp_nshv_b",
    service_name = "NSHV B Temperatur",
    discovery_function = discover_temp_nshv_b,
    check_function = check_temp_nshv_b,
)
