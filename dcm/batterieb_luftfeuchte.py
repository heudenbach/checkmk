...
def discover_batterieb_luftfeuchte(section):
    yield Service()

def check_batterieb_luftfeuchte(section):
...
register.check_plugin(
    name = "batterieb_luftfeuchte",
    service_name = "Batterie-B Luftfeuchtigkeit",
    discovery_function = discover_batterieb_luftfeuchte,
    check_function = check_batterieb_luftfeuchte,
)
...
register.snmp_section(
    ...
    exists(".1.3.6.1.3.1.0.25")
        ...
        base = '.1.3.6.1.3.1.2',
        oids = [
            '1.0',    # Batterie-B Luftfeuchtigkeit
        ],
    ),
)