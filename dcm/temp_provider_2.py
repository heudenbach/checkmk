...
def discover_temp_provider_2(section):
    yield Service()

def check_temp_provider_2(section):
    ...
    yield Result(
        state=state,
        summary="Temperatur Provider 2: % 6.1f° Celcius" % (aussen_temp)
    )
    return

register.check_plugin(
    name = "temp_provider_2",
    service_name = "Provider 2 Temperatur",
    discovery_function = discover_temp_provider_2,
    check_function = check_temp_provider_2,
)
