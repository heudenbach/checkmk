...
def discover_temp_usv_a(section):
    yield Service()

def check_temp_usv_a(section):
    ...
    yield Result(
        state=state,
        summary="Temperatur USV-A: % 6.1f° Celcius" % (aussen_temp)
    )
    return

register.check_plugin(
    name = "temp_usv_a",
    service_name = "USV-A Temperatur",
    discovery_function = discover_temp_usv_a,
    check_function = check_temp_usv_a,
)
