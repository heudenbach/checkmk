...
def discover_ukg_uv_usv_a_sammelst(section):
    yield Service()

def check_ukg_uv_usv_a_sammelst(section):
...
register.check_plugin(
    name = "ukg_uv_usv_a_sammelst",
    service_name = "UKG UV-USV-A Sammelstörung",
    discovery_function = discover_ukg_uv_usv_a_sammelst,
    check_function = check_ukg_uv_usv_a_sammelst,
)
...
register.snmp_section(
    ...
    exists(".1.3.6.1.3.1.0.25")
        ...
        base = '.1.3.6.1.3.1.3',
        oids = [
            '4.0',    # UKG UV-USV-A Sammelstörung
        ],
    ),
)