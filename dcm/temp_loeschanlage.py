...
def discover_temp_loeschanlage(section):
    yield Service()

def check_temp_loeschanlage(section):
    ...
    yield Result(
        state=state,
        summary="Temperatur Löschanlage: % 6.1f° Celcius" % (aussen_temp)
    )
    return

register.check_plugin(
    name = "temp_loeschanlage",
    service_name = "Löschanlage Temperatur",
    discovery_function = discover_temp_loeschanlage,
    check_function = check_temp_loeschanlage,
)
