...
def discover_loeschan_luftfeuchte(section):
    yield Service()

def check_loeschan_luftfeuchte(section):
...
register.check_plugin(
    name = "loeschan_luftfeuchte",
    service_name = "Löschanlage Luftfeuchtigkeit",
    discovery_function = discover_loeschan_luftfeuchte,
    check_function = check_loeschan_luftfeuchte,
)
...
register.snmp_section(
    ...
    exists(".1.3.6.1.3.1.0.25")
        ...
        base = '.1.3.6.1.3.1.1',
        oids = [
            '3.0',    # Löschanlage Luftfeuchtigkeit
        ],
    ),
)